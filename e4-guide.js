...
#define TIME_LOCATION_PPS_SENSE       OFF        // HIGH if your GPS has a PPS pin</pre>
  ...
  C.firmware = () => {
    ...
    return `
      ...
      ${callout('info', 'Easier option: use the <a href="https://graydigitalarts.com/OnStep-Web-Tools/" target="_blank" rel="noopener">OnStep ESP32 Web Installer</a> for browser-based flashing. Below is the manual method.')}
      ${callout('info', `<strong>Community recipe — systematic first light</strong> (the fastest way to isolate problems on a new board): <ol style="margin-top:6px;line-height:1.8"><li>Strip everything: only <strong>12V</strong> + USB connected, <strong>no steppers</strong>, all factory shunts off, Z-MIN→PDN jumper on.</li><li>Flash Chad's pre-compiled E4 build via the <a href="https://graydigitalarts.com/OnStep-Web-Tools/" target="_blank" rel="noopener">web installer</a> (rules out Arduino/library issues).</li><li>Power off, connect <strong>one</strong> motor to <strong>MOT-X (RA)</strong>; set the PSU current limit to ≥3A.</li><li>Power on, connect to the "OnStepX" WiFi → 192.168.0.1, and test that one axis before adding the rest.</li></ol><span style="color:var(--e4-dim)">Source: <a href="https://onstep.groups.io/g/main/message/66848" target="_blank" rel="noopener">#66848</a></span>`)}
