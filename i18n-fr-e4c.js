...
window.I18N_FR = Object.assign(window.I18N_FR || {}, {
  ...
  /* ------------------------------------------- limits, verified vs. source */
  "Per Config.h: OFF disables limits until an unpark goto or a sync; ON enables them at startup (the defaults file adds \"if date/time are set\"). The stock E4 Config.h ships OFF.": "D'après Config.h : OFF désactive les limites jusqu'à un goto de déparcage ou une synchro ; ON les active au démarrage (le fichier de valeurs par défaut précise « si la date/heure sont réglées »). Le Config.h E4 d'origine est livré sur OFF.",
  "When any limit triggers: all gotos abort, tracking stops, the mount freezes. Recover by clearing the condition (move off the switch) and then unparking or syncing — with LIMIT_STRICT OFF, an unpark goto or a sync re-arms normal operation.": "Quand une limite se déclenche : tous les gotos sont interrompus, le suivi s'arrête, la monture se fige. Pour récupérer, supprimez la condition (dégagez le contacteur) puis déparquez ou synchronisez — avec LIMIT_STRICT sur OFF, un goto de déparcage ou une synchro réarme le fonctionnement normal.",
  "Active LOW — short to GND = limit triggered. Ships OFF, so limits do nothing until you set this": "Actif à l'état bas — mise à la masse = limite déclenchée. Livré sur OFF, les limites ne font donc rien tant que vous ne réglez pas ce paramètre",
  "Already set to 34 in the stock E4 Config.h, overriding the pinmap default of 39 (TB)": "Déjà réglé sur 34 dans le Config.h E4 d'origine, ce qui remplace la valeur 39 (TB) par défaut du pinmap",
  "Stock value. OFF = limits off until an unpark goto or sync; ON = armed at startup": "Valeur d'origine. OFF = limites inactives jusqu'à un goto de déparcage ou une synchro ; ON = armées au démarrage",
  "Restores where the mount was pointing after a power cycle. Default OFF. Uses NV storage, not the RTC — but it writes often, so FRAM is kinder than flash-backed EEPROM": "Restaure la direction dans laquelle pointait la monture après une coupure. OFF par défaut. Utilise la mémoire NV, pas l'horloge RTC — mais l'écriture est fréquente, une FRAM est donc plus ménageante qu'une EEPROM émulée en flash",
